...
*** Project Management
    ...
    This setup allows me to have a single active project at a time. I
    can find files "within" this project with a fuzzy search. I can
    get a "sidebar" that shows me the directory structure of the
    project. I can have an "instance" of emacs for each project. Emacs
    discovers which projects exist by looking through known places on
    my file system, so, from a project creation point of view, I just
    clone projects into one of several directories and re-run a
    function in emacs (or restart it).

    All of this is achieved, primarily, by software written by
    [[https://github.com/rdallasgray][rdallasgray]] and [[https://github.com/bbatsov][bbatsov]].

    [[https://github.com/rdallasgray/project-persist][Project-persist]] defines what a project is, and provides hooks
    around creating, opening and closing them. [[https://github.com/rdallasgray/graphene][Graphene]] has a bunch of
    tweaks to the sidebar (and some other neat stuff).

    [[https://github.com/bbatsov/projectile][Projectile]] helps me find files within a project.

**** projectile

     It's presence is enough to get going. We'll setup the fuzzy-find later.
...
***** Hooks
      ...
      These are setup in the section graphene-projects.el below.

***** Project-persist and Projectile Integration

      These two projects integrate using a well-known global variable
      named =default-directory=. If project-persist sets this variable
      in hooks mentioned above (and below), then projectile will pick
      up on it and focus searches underneath it.
...
**** graphene
     ...
     Graphene is a great package, but, like many packages, I like to
     split it up and choose the parts I'm most interested in. It's too
     large for me to subsume at once. Consequently, the following
     section contains parts of the code that I'm using. I reference
     the original file names to keep stuff clear(er). I am going
     through this code and pruning out stuff I don't use, or moving
     stuff I do use to its appropriate home in my emacs-setup.

     For now, the following three sections are the heart of my project
     management solution.
