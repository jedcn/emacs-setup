...
** Magit
   ...
   I use some elisp from the ESK to color additions and removals
   red/green.
   ...
   Also, I happen to have two =emacsclient=s, one at =/usr/bin= and
   another at =/usr/local/bin=. The one at =/usr/bin= cannot find my
   emacs server and this causes Magit to freeze whenever I try to
   commit. This is why I explicitly set
...
* Appendix B: Babel and the Config
  ...
  The remainder of this file details how this orchestration works. All
  of the functions and variables in this section begin with
...
** Concatenation
   ...
   We build the composite file with =create-composite-org=, and it in
